    ...
    while(1) {
        if (accepting_name) {
            ...
            if ((PIND & (1 << PIND4))) {
                ...
                if (messageBuf[3] >= '0' && messageBuf[3] <= '9') {
                    int punch = 1000 * (messageBuf[0] - '0') + 100 * (messageBuf[1] - '0') + 10 * (messageBuf[2] - '0') + 1 * (messageBuf[3] - '0');

                    // Punch threshold
                    if (punch > 1300 && punchable == 1) {
                        // Make unpunchable
                        punchable = 0;
                        // Scale punches to be three digit scores
                        punch = punch / 7;
                        // Update "Your Score"
                        LCD_drawBlock(120, 0, 159, 10, rgb565(255, 255, 255));
                        ...
                        // Since the sensor dropped back below the threshold, make punchable again.
                        //_delay_ms(5000);
                        punchable = 1;
                    }
                }
